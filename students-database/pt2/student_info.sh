...
echo "$($PSQL "SELECT course FROM courses JOIN majors_courses USING(course_id) JOIN students ON majors_courses.major_id = students.major_id GROUP BY course HAVING COUNT(students.student_id) = 1 ORDER BY course;")"
